...
class M_cuti extends CI_Model
{
  ...
  public function cuti_divisi()
  {
    ...
    foreach ($sql as $s) :
      ...
      if ($id == 3) {
        //anisa
        $this->db->where('nama_id', 115);
        // $this->db->or_where('nama_id', 3);
        $this->db->or_where('nama_id', 8);
        $this->db->or_where('nama_id', 36);
        $this->db->or_where('nama_id', 134);
        $this->db->or_where('nama_id', 138);
        ...
        //tia
        $this->db->where('nama_id', 105); //rizky
        return $this->db->order_by('id', 'Desc')->get('cuti');
      } elseif ($id == 105) {
        //rizky
        ...
        $this->db->or_where('nama_id', 25); //felix
        // $this->db->or_where('nama_id', 105); //rizky
        $this->db->or_where('nama_id', 136); //surya dimas
        $this->db->or_where('nama_id', 140); //margaret
        return $this->db->order_by('id', 'Desc')->get('cuti');
      } elseif ($id == 2) {
        //bunga
  ...
  {
    $newCode = $this->input->post('kode_cuti', true);
    $idhead = $this->input->post('approve', true);
    $status = $this->input->post('status', true);
    $tanggal = $this->input->post('date_created', true);
    ...
    if ($this->input->post('value', true) == 1) {
      ...
      $data = [
        ...
        'head' => htmlspecialchars($this->input->post('approve', true)),
        'durasi' => $x + 1
      ];
      $this->db->where('id', $id);
      $this->db->update('cuti', $data);
      // $this->_sendEmailHead($post, $id_nama);
      $this->_history($newCode, $status, $idhead, $tanggal, $catatan);
    } else {
      $id = $this->input->post('id', true);
      ...
      $data = [
        ...
        'date_acc_hr' => htmlspecialchars($this->input->post('date_acc_hr', true)),
        'status' => htmlspecialchars($this->input->post('status', true)),
        'catatan_hr' => htmlspecialchars($this->input->post('catatan', true)),
        'hr' => htmlspecialchars($this->input->post('approve', true)),
        'durasi' => $x + 1
      ];
      $this->db->where('id', $id);
      $this->db->update('cuti', $data);
  ...
  public function tambah_cuti()
  {
    ...
    $tanggal =  htmlspecialchars($this->input->post('date_created', true));
    $awal =  htmlspecialchars($this->input->post('tanggal', true));
    $akhir = htmlspecialchars($this->input->post('date_end', true));
    $id = htmlspecialchars($this->input->post('nama_id', true));
    $status = htmlspecialchars($this->input->post('status', true));
    $catatan = htmlspecialchars($this->input->post('keterangan', true));
    ...
    $this->_history($newCode, $status, $id, $tanggal, $catatan);
  ...
  private function _history($newCode, $status, $id, $tanggal, $catatan)
  {
    $data = [
      'kode_cuti' => $newCode,
      'status' => $status,
    ...
    $this->db->insert('history_cuti', $data);
  }
}
