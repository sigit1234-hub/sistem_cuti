...
class Hrd extends CI_Controller
{
    ...
    public function print($kode_spl)
    {
        $data['title'] = "Surat Perintah Lembur";
        $data['user'] = $this->User_m->tampil_data();
        $data['karyawan'] = $this->User_m->ambil_karyawan($kode_spl);

        $data['spl'] = $this->M_spl->tampil_data_spl($kode_spl);

        $data['divisi'] = $this->M_spl->divisi_data();
        $data['anggota'] = $this->M_spl->data_spl($kode_spl);
        $data['id'] = $kode_spl;
